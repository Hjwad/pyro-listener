...
class Listener:
    ...
    async def listen(
        ...
        if self.client.name not in _cache:
            _cache[self.client.name] = {'list': []}

        if data in _cache[self.client.name]['list']: 
        ...
        def ___():
            while data in _cache[self.client.name]['list']:
                if (timeout) and (datetime.now() > stamp):
                    if json.dumps(data, ensure_ascii=False) in _cache[self.client.name]:
                        del _cache[self.client.name][json.dumps(data, ensure_ascii=False)]
                    _cache[self.client.name]['list'].remove(data)
                    raise TimeOut("Time out error")
                sleep(0.1)
            return _cache[self.client.name].get(json.dumps(data, ensure_ascii=False))
    ...
    async def _handler(self, client: pyrogram.Client, message: pyrogram.types.Message):
        ...
        if not client_name or client_name not in _cache:
            return 
        ...
        for data in list(_cache[client_name]['list']):
            if (data['chat_id'] == chat_id) and (
                (data["from_id"] is None) or 
                (isinstance(data["from_id"], list) and sender.id in data["from_id"]) or 
                ...
                if data["filters_type"] == 2:
                    for _ in data["filters"]:
                        if hasattr(message, _) and getattr(message, _):
                            __.append(_)
                    if __ != data["filters"]: continue

                cache_key = json.dumps(data, ensure_ascii=False)
                if self.show_output:
                    message.output = _cache[client_name].get(cache_key)
                
                _cache[client_name][cache_key] = message
                _cache[client_name]['list'].remove(data)
                break
